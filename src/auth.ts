        ...
        Credentials({
            ...
            async authorize(credentials: { identifier: string; password: string }): Promise<User | null> {
                ...
                if (existingUser.length === 0) {
                    ...
                    }).returning({ id: users.id, email: users.email, username: users.username, role_id: users.role_id });

                    return newUser ? { id: newUser.id, email: newUser.email, username: newUser.username, role_id: newUser.role_id } : null;
                } else {
                    const user = existingUser[0];
                    const isPasswordValid = await bcrypt.compare(credentials.password, user.password ?? "");
                    if (!isPasswordValid) return null;
                    return { id: user.id, email: user.email, username: user.username, role_id: user.role_id };
                }
            },
        }),
    ],
